...
import { X, CalendarPlus, Store } from 'lucide-react';
      ...
      <div className="safe-bottom absolute bottom-0 left-0 right-0 max-h-[85vh] overflow-y-auto rounded-t-sheet bg-surface p-6 shadow-sheet">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Link
            ...
            className="flex flex-col gap-2 rounded-card border border-border bg-surface-card p-3 transition-fast hover:bg-border/40 active:scale-[0.98]"
          >
            <div className="flex h-12 w-12 items-center justify-center rounded-card bg-corail/10 text-corail">
              <Store size={22} />
            </div>
            <div>
              <p className="text-sm font-semibold text-corail">Commerce</p>
              <p className="mt-0.5 text-xs text-content-secondary">Référencez un commerce ou un lieu</p>
            </div>
          </Link>
        </div>
      </div>
