            ...
            {feed.map((post) => {
              ...
              return (
                <Link
                  ...
                  className="block overflow-hidden rounded-card bg-surface-card p-2 shadow-soft"
                >
                  <div className="relative h-24 w-full overflow-hidden rounded-card bg-surface">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={thumbnail} alt="" className="h-full w-full object-cover" />
                    <FavoriteHeartButton postId={post.id} />
                  </div>

                  <span className="mt-2 inline-block rounded-pill bg-surface px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-content-secondary">
                    {typeInfo.label}
                  </span>
