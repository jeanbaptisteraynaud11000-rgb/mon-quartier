#!/usr/bin/env bash
set -e
echo "Fix : useSearchParams necessite un Suspense pour le build de production..."

mkdir -p "src/app/login"
cat > "src/app/login/page.jsx" << 'MQEOF_SRC_APP_LOGIN_PAGE_JSX'
'use client';
...
MQEOF_SRC_APP_LOGIN_PAGE_JSX

mkdir -p "src/app/email-confirmation"
cat > "src/app/email-confirmation/page.jsx" << 'MQEOF_SRC_APP_EMAIL-CONFIRMATION_PAGE_JSX'
'use client';
...
MQEOF_SRC_APP_EMAIL-CONFIRMATION_PAGE_JSX
...
echo "Prochaine etape : git add -A && git commit -m \"fix: suspense boundary pour useSearchParams (build Vercel)\" && git push"