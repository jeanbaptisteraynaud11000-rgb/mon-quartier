...
export default async function HomePage() {
  ...
  const [neighborsCount, postsTodayCount, entraideCount, feedResult, upcomingEventsResult] = await Promise.all([
    supabase
      .from('profiles')
  ...
  return (
    <div className="flex flex-col gap-6 p-4">
      ...
      {/* Prochaines activités — rubrique distincte du fil d'annonces */}
      {upcomingEvents.length > 0 && (
        <section>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-base font-semibold text-content-primary">Prochaines activités</h2>
            <Link href="/activites" className="text-sm font-medium text-corail">
              Tout voir
            </Link>
          </div>
            ...
            {upcomingEvents.map((event) => {
              ...
              return (
                <Link
                  ...
                  className="flex w-52 flex-shrink-0 flex-col gap-2 rounded-card border border-border bg-surface-card p-4 shadow-soft transition-fast hover:bg-border/20 active:scale-[0.98]"
                >
                  <div className="flex h-10 w-10 items-center justify-center rounded-pill bg-vert/10 text-vert">
                    <Icon size={18} />
                  </div>
                  <p className="text-xs font-medium text-content-secondary">
                    {formatEventDate(event.event_date)}
                  </p>
                  <p className="line-clamp-2 font-semibold text-content-primary">{event.title}</p>
                  <p className={`text-xs font-medium ${isFull ? 'text-corail' : 'text-content-secondary'}`}>
