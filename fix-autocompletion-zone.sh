  ...
  function handleZoneChange(value) {
    ...
    zoneDebounceRef.current = setTimeout(async () => {
      try {
        ...
        // Garde-fou : si le texte a changé depuis le lancement de cette
        // requête (l'utilisateur a continué à taper), on ignore cette
        // réponse potentiellement obsolète plutôt que d'écraser les
        // suggestions actuelles avec des résultats qui ne correspondent
